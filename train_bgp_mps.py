...
batch_size_train = 4
num_samples_per_epoch_train = 32
...
if __name__ == '__main__':
    ...
    dataset = FactBaseSemanticsDataset(sem, "bgp-ospf-dataset-sub", num_samples=NUM_LEN_DATASET) # !!!! generate (or load) the dataset
    ...
    for epoch in tqdm(range(args.epochs), leave=False):
        ...
        for i, batch in tqdm(enumerate(training_loader), leave=False, total=num_batches_per_epoch, desc=f"Epoch {epoch}"):
            ...
            target = mask_node_features(batch.x, mask.logical_not()) # 是 batch.x 的精简版，只包含需要预测的部分

            # 3. 前向传播 ← 这里执行 graph regression
            x_emb = model.forward(
                batch.x, 
                mask, # mask 是一个布尔矩阵，告诉模型**“哪些值需要被盖住”**
                batch.edge_index, 
                batch.edge_type, 
                False)
            ...
            for f in synthesised_features: 
                loss += model.decoder.loss(x_emb, target, f.name)

            if torch.any(torch.isnan(loss)): print("isnan") # 检测训练是否出错 --> 具体是检查损失值是否变成了 NaN (Not a Number)
