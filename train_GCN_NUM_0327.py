...
if __name__ == '__main__':
    def get_args():
        ...
        parser.add_argument("--feature", dest="feature", type=str, default=None)  # ← 加这一行
    ...
    # 1. 创建输出目录
    # ——————————————————————————————————————————————————
    feat_name = synthesised_features[0].name
    base_dir = f"results/{feat_name}"
    os.makedirs(base_dir, exist_ok=True)

    # 自动找下一个编号
    existing = [d for d in os.listdir(base_dir) if d.isdigit()]
    next_run = max([int(d) for d in existing], default=0) + 1
    out_dir = f"{base_dir}/{next_run}"
    ...
    csv_path = f"{out_dir}/metrics_GCN_NUM_{feat_name}.csv"
    ...
    ckpt_path = f"{out_dir}/model_GCN_NUM_{feat_name}.pt"
    torch.save({
        'feature': feat_name,
    ...
    ax1.set_title(f'Loss — {feat_name}')
    ax1.set_xlabel('Batch Step');
    ax1.set_ylabel('Loss');
    ax1.grid(True)

    ax2.plot(df['accuracy'], color='orange')
    ax2.set_title(f'Accuracy — {feat_name}')
    ax2.set_xlabel('Batch Step');
    ax2.set_ylabel('Accuracy');
    ax2.grid(True)

    ax3.plot(df['f1'], color='green')
    ax3.set_title(f'F1 Score — {feat_name}')
    ax3.set_xlabel('Batch Step');
    ax3.set_ylabel('F1');
    ax3.grid(True)

    plt.tight_layout()
    fig_path = f"{out_dir}/fig_GCN_NUM_{feat_name}.pdf"
